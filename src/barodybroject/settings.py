...
IS_PRODUCTION = env.bool('RUNNING_IN_PRODUCTION', default=False)  # Default to development
...
SECRET_KEY_FALLBACK = env.str('DJANGO_SECRET_KEY_DEV_FALLBACK', default='dev-only-insecure-key-change-in-production')
SECRET_KEY = secrets.get("DJANGO_SECRET_KEY") or env.str('SECRET_KEY', default=SECRET_KEY_FALLBACK)
...
CONTAINER_APP_NAME = (
    env.str('CONTAINER_APP_NAME', default='') or 
    secrets.get("CONTAINER_APP_NAME", '') or 
    "barodybroject"
)
CONTAINER_APP_ENV_DNS_SUFFIX = (
    env.str('CONTAINER_APP_ENV_DNS_SUFFIX', default='') or
    secrets.get("CONTAINER_APP_ENV_DNS_SUFFIX", '') or
    "com"
)
AZURE_CONTAINER_REGISTRY_ENDPOINT = (
    env.str('AZURE_CONTAINER_REGISTRY_ENDPOINT', default='') or
    secrets.get("AZURE_CONTAINER_REGISTRY_ENDPOINT", '') or
    "https://barodybroject.azurecr.io"
)

# GitHub integration
GITHUB_ISSUE_REPO = (
    env.str('GITHUB_ISSUE_REPO', default='') or 
    secrets.get("GITHUB_ISSUE_REPO", '')
)
...
INSTALLED_APPS = DJANGO_APPS + CMS_APPS + AUTH_APPS + THIRD_PARTY_APPS + LOCAL_APPS + DEV_APPS
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            ...
            'loaders': [
                ...
                ]) if IS_PRODUCTION else 'django.template.loaders.filesystem.Loader',
                'django.template.loaders.app_directories.Loader' if not IS_PRODUCTION else None,
...
if IS_PRODUCTION:
    ...
    EMAIL_USE_TLS = True
    DEFAULT_FROM_EMAIL = env.str('DEFAULT_FROM_EMAIL', default='no-reply@barodybroject.com')
...
ALLOWED_HTML_TAGS = [
    ...
    'hr', 'i', 'img', 'ins', 'kbd', 'label', 'legend',
    'li', 'ol', 'p', 'pre', 'small', 'span', 'strong',
