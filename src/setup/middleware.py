...
class InstallationMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if self._is_exempted_path(request.path):
            logger.debug(
                f"Skipping installation check for exempted path: {request.path}"
            )
            return None

        # Skip in debug mode for development (optional)
        if getattr(settings, "DEBUG", False) and getattr(
            settings, "SKIP_INSTALLATION_CHECK", False
        ):
        ...
        try:
            ...
            if not self.installation_service.is_installation_complete():
                logger.info(
                    f"Installation incomplete, redirecting {request.path} to setup wizard"
                )
...
class SetupRequiredMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not installation_service.is_installation_complete():
            logger.warning(
                f"Access denied to {self.__class__.__name__}: installation incomplete"
            )
...
def setup_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not installation_service.is_installation_complete():
            logger.warning(
                f"Access denied to {view_func.__name__}: installation incomplete"
            )
