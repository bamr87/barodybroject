...
class AssistantAdmin(ImportExportModelAdmin):
    ...
    def fetch_openai_assistants(self, request, queryset):
        try:
            ...
            for (
                ...
                assistant_id = (
                    assistant_data.id
                )  # Changed from assistant_data['id'] to assistant_data.id
                ...
                if hasattr(assistant_data, "response_format") and hasattr(
                    assistant_data.response_format, "json_schema"
                ):
            ...
            self.message_user(
                request,
                "Successfully synchronized assistants from OpenAI.",
                messages.SUCCESS,
            )
        except Exception as e:
            self.message_user(
                request, f"Error fetching assistants: {e}", messages.ERROR
            )

    fetch_openai_assistants.short_description = (
        "Fetch and synchronize assistants from OpenAI"
    )
    ...
    def delete_model(self, request, obj):
        try:
            ...
            self.message_user(
                request,
                f"Assistant '{obj.name}' deleted successfully.",
                messages.SUCCESS,
            )
        except Exception as e:
            self.message_user(
                request, f"Error deleting assistant '{obj.name}': {e}", messages.ERROR
            )
    ...
    def delete_queryset(self, request, queryset):
        ...
        for obj in queryset:
            try:
                ...
                self.message_user(
                    request,
                    f"Error deleting assistant '{obj.name}': {e}",
                    messages.ERROR,
                )
        if not errors:
            self.message_user(
                request, "Selected assistants deleted successfully.", messages.SUCCESS
            )
...
class OpenAIModelAdmin(ImportExportModelAdmin, admin.ModelAdmin):
    ...
    def fetch_openai_models(self, request, queryset):
        try:
            ...
            self.message_user(
                request,
                "Successfully fetched and saved model choices",
                messages.SUCCESS,
            )
        except Exception as e:
