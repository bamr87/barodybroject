...
every GitHub failure during publication is raised as a `PublicationError` carrying a reader-facing message, so `ManagePostView.publish()` answers with a `messages.error` and a redirect to `manage_post` rather than a 500 (issue #114)
