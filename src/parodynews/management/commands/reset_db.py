...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        db_path = 'db.sqlite3'  # Adjust this path if your database file is located elsewhere
        ...
        for migrations_dir in migrations_dirs:
            ...
            for migration_file in migration_files:
                if os.path.basename(migration_file) != '__init__.py':
                    ...
                    self.stdout.write(self.style.SUCCESS(f"Deleted migration file: {migration_file}"))

            # Also delete the compiled Python files
            migration_pyc_files = glob.glob(os.path.join(migrations_dir, '__pycache__', '*.pyc'))
            for migration_pyc_file in migration_pyc_files:
                os.remove(migration_pyc_file)
                self.stdout.write(self.style.SUCCESS(f"Deleted compiled migration file: {migration_pyc_file}"))
